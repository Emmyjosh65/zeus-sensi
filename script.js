...
function generateAI(){
  ...
  playSound(clickSound);
  document.getElementById('aiLoading').classList.remove('hidden');
  playSound(aiSound); // AI thinking sound loop
...
function showResult(){
  ...
  document.getElementById('resultSection').classList.remove('hidden');
  const g = Math.floor(Math.random()*30)+70;
  const device = document.getElementById('model').value;
  ...
  const resultHTML = `
    ...
    <div class="glass" style="padding:20px"><h3>4X Scope</h3><p>${g-5}</p></div>
    <div class="glass" style="padding:20px"><h3>Sniper</h3><p>${g-15}</p></div>
    <div class="glass" style="padding:20px"><h3>DPI</h3><p>${650+Math.floor(Math.random()*200)}</p></div>
    <div class="glass" style="padding:20px"><h3>Headshot Accuracy</h3><p>${92+Math.floor(Math.random()*5)}%</p></div>
    <div class="glass" style="padding:20px"><h3>Gyroscope</h3><p>Optimized</p></div>
    <div class="glass" style="padding:20px"><h3>Device Temperature</h3><p>Cool</p></div>
    <div class="glass" style="padding:20px"><h3>Network</h3><p>Low Ping</p></div>
    <div class="glass" style="padding:20px;grid-column:1/-1"><h3>Pro Tip</h3><p>Use ${document.getElementById('fingers').value} claw for better drag. Keep graphics on Smooth for max FPS.</p></div>
  ...
  // AI VOICE READS RESULTS
  const voiceText = `Zeus AI has generated your sensitivity for ${device}. 
  General sensitivity ${g}. Red Dot ${g+15}. 
  Headshot Accuracy ${92+Math.floor(Math.random()*5)} percent. 
  Recommendation: Use ${document.getElementById('fingers').value} claw for better drag.`;
...
function copySettings(){playSound(clickSound); navigator.clipboard.writeText(document.getElementById('resultGrid').innerText)}
function shareSettings(){playSound(clickSound); navigator.share? navigator.share({title:'ZEUS SENSI'}) : copySettings()}
function downloadScreenshot(){playSound(clickSound); window.print()}
function saveFav(){playSound(successSound); alert('Saved to Favorites')}
function compareDevice(){playSound(clickSound); alert('Compare feature coming soon')}
function randomGen(){playSound(clickSound); alert('Random sensitivity generated')}
