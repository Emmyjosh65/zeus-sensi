...
// FIX 1: Wait for page to load before running canvas
window.onload = function(){
  ...
  if(canvas){
    ...
    let particlesArray = [];
    for(let i=0;i<30;i++){particlesArray.push({x:Math.random()*canvas.width,y:Math.random()*canvas.height,size:Math.random()*2+1,speedX:Math.random()*1-0.5,speedY:Math.random()*1-0.5})}
...
document.addEventListener('mousemove',e=>{
  ...
  if(glow){glow.style.left=e.pageX+'px';glow.style.top=e.pageY+'px'}
})
...
function selectDevice(name){
  ...
  document.getElementById('deviceSpecs').innerHTML = `<h3>${name}</h3><p>Optimized for Headshots</p>`;
}
function startAI(){
  showPage('ai');
...
function generateResult(){
  ...
  // FIX 2: Fixed syntax error here. Was [k][v] now [k,v]
...
function exportPNG(){ toast('Exported PNG ✅'); }
